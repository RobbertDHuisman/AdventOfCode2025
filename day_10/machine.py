...
class Machine:
    ...
    def reach_joltage(self):
        ...
        # print(f"target joltage: {self.joltage}")
        # print(f"buttons: {self.buttons}, joltage_buttons: {joltage_buttons}, target joltage: {self.joltage}")

        # Create the LP problem
        prob = LpProblem("Minimize_Button_Presses", LpMinimize)
        
        # Create variables (one for each button)
        x = [LpVariable(f"button_{i}", lowBound=0, cat='Integer') for i in range(len(joltage_buttons))]
        
        # Objective function: minimize sum of button presses
        prob += lpSum(x)
        
        # Constraints: for each joltage dimension, sum must equal target
        for dim in range(len(self.joltage)):
            prob += lpSum([joltage_buttons[btn][dim] * x[btn] for btn in range(len(joltage_buttons))]) == self.joltage[dim]
        
        # Solve the integer linear program
        prob.solve()
        
        if LpStatus[prob.status] == 'Optimal':
            self.joltage_buttons_pressed = int(sum([x[i].varValue for i in range(len(x))]))  
            self.button_presses = np.array([int(x[i].varValue) for i in range(len(x))])
        ...
        A = np.array(joltage_buttons).T  # Transpose to get equations x variables
        b = np.array(self.joltage)
        
        # Minimize sum of button presses: min(x1 + x2 + ... + xn)
        c = np.ones(len(joltage_buttons))  # Coefficients for objective function
        ...
        integrality = np.ones(len(joltage_buttons))
        
        # Solve the integer linear program
        result = milp(c, constraints=constraints, bounds=bounds, integrality=integrality)
        
        if result.success:
            self.joltage_buttons_pressed_scipy = int(result.fun)
            self.button_presses_scipy = result.x.astype(int)
            # print(f"Minimum button presses: {self.joltage_buttons_pressed_scipy}")
            # print(f"Button press counts: {self.button_presses_scipy}")
        else:
            print("No solution found")
            # self.joltage_buttons_pressed_scipy = None
